...
specific MCP and nothing else), add these fields to the profile. They are
honored by `brain-subagent-dispatch` (gemini branch) — the canonical and only
authoritative consumer. Absent fields mean "no restriction."

| Field | Type | Purpose |
|---|---|---|
| `allowed_tools` | list[str] | Built-in tool names the sub-agent may call. Enforce via `policy_file`. |
| `allowed_mcp_servers` | list[str] | MCP server names allowed. Passed as `--allowed-mcp-server-names` to gemini. |
| `allowed_extensions` | list[str] | CLI extensions to load (reduces context). Passed as `-e` to gemini. |
| `policy_file` | str | Path (relative to `$BRAIN` or absolute) to a Gemini Policy Engine TOML; denied tools are excluded from the model's registry. |
| `neutral_cwd` | str | Directory to `cd` into before invoking the runtime. Escapes project-scoped runtime hooks and commands. |
| `lifecycle_blocked` | bool | When `true` (default when absent), consumer sets `BRAIN_DISPATCH_ACTIVE=1`, causing lifecycle scripts (`brain-meditate`, `brain-checkpoint`, `brain-handoff`, `brain-session-end-commit`) to refuse execution. Set `false` to opt out. |
| `memory_mode` | str | `read-only` / `none` / `full`. Advisory for runtimes that support it. |
| `approval_mode` | str | Passed as `--approval-mode` to gemini. `plan` (read-only, dispatch default when absent) / `yolo` (auto-approve — required for non-interactive MCP calls) / `default`. |
...
3. If the profile should run as a scoped sub-agent, add an `isolation_fields`-compatible set: `allowed_mcp_servers`, `allowed_extensions`, `policy_file`, `neutral_cwd`, `approval_mode`, and any applicable `lifecycle_blocked` / `memory_mode` entries.
...
summary when any isolation field is set on a gemini-targeted profile.
